...
###
DinchiAdminApp.filter "propsFilter", ->
  (items, props) ->
    out = []
...
DinchiAdminApp.controller "UISelectController", ($scope, $http, $timeout) ->
  ...
  $scope.someGroupFn = (item) ->
    return "From A - M"  if item.name[0] >= "A" and item.name[0] <= "M"
    "From N - Z"  if item.name[0] >= "N" and item.name[0] <= "Z"
